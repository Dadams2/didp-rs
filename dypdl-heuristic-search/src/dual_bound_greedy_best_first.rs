...
/// Creates a Greedy Best-First Search solver using the dual bound as a heuristic function.
///
...
///
/// Greedy Best-First Search uses only the heuristic function (h) to guide the search, ignoring the path cost (g).
/// This makes it faster but potentially suboptimal compared to A* search.
/// It expands nodes in order of their heuristic value only.
///
/// # References
///
...
/// ```
/// use dypdl::prelude::*;
/// use dypdl_heuristic_search::{create_dual_bound_greedy_best_first, FEvaluatorType, Parameters};
/// use std::rc::Rc;
///
...
///
/// let mut solver = create_dual_bound_greedy_best_first(model, parameters, f_evaluator_type);
/// let solution = solver.search().unwrap();
/// assert_eq!(solution.cost, Some(1));
/// assert_eq!(solution.transitions, vec![increment]);
/// assert!(!solution.is_infeasible);
/// ```
pub fn create_dual_bound_greedy_best_first<T>(
    ...
    <T as str::FromStr>::Err: fmt::Debug,
{
    ...
    if model.has_dual_bounds() {
        ...
        // Greedy Best-First Search: f(n) = h(n), ignore g(n)
        let f_evaluator = move |_g, h, _: &_| h;
        ...
        // Fallback to cost-only search if no dual bounds are available
...
mod tests {
    ...
    fn test_greedy_best_first_search() {
        ...
        model.add_forward_transition(increment.clone()).unwrap();
        model
            .add_dual_bound(IntegerExpression::from(0))
            .unwrap();
    ...
    #[test]
    fn test_greedy_best_first_search_without_dual_bound() {
        let mut model = Model::default();
        let variable = model.add_integer_variable("variable", 0).unwrap();
        model
            .add_base_case(vec![Condition::comparison_i(
