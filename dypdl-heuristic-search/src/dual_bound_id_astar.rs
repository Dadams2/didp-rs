    ...
    <T as str::FromStr>::Err: fmt::Debug,
{
    ...
    if model.has_dual_bounds() {
        ...
        // Without dual bounds, IDA* doesn't make sense, so fall back to best-first search
...
/// where `cost` is `IntegerExpression::Cost`or `ContinuousExpression::Cost` and `w` is a numeric expression independent of `cost`.
///
/// IDA* performs iterative deepening depth-first search with f-cost thresholds.
/// Each iteration explores all nodes with f-cost up to the current threshold.
/// The next threshold is the minimum f-cost of nodes that exceeded the current threshold.
///
    ...
    Transition: From<TransitionWithId<V>>,
{
    ...
    fn dfs_threshold(&mut self, node: Rc<N>, threshold: T) -> Option<T> {
        ...
        if let Some(f_cost) = node.bound(&model) {
            ...
            // For minimization, better bounds are more negative
            // For maximization, better bounds are more positive
            ...
            if exceeds_threshold {
                ...
                // For minimization, we want the maximum (least negative) of exceeding bounds
                // For maximization, we want the minimum of exceeding bounds
        ...
        // Sort successors by f-cost (best first for efficiency)
        successors.sort();
    ...
    Transition: From<TransitionWithId<V>>,
{
    fn search_next(&mut self) -> Result<(Solution<T>, bool), Box<dyn Error>> {
        ...
        while let Some(current_threshold) = self.threshold {
            ...
            // Clear the registry for the next iteration while keeping capacity
            self.registry.clear();
            ...
            if let Some(next_t) = self.next_threshold {
                ...
                self.solution.time = self.time_keeper.elapsed_time();
                update_bound_if_better(&mut self.solution, next_t, &self.generator.model, self.quiet);
            } else {
